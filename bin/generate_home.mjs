...
const targets = {
  ...
  analysis: join(docsDir, 'reports', 'analysis'),
...
const latest = {
  ...
  analysis: latestMdName(targets.analysis),
...
lines.push(`- CC股票研究工具：${latest.analysis ? `[./reports/analysis/${latest.analysis}](./reports/analysis/${latest.analysis})` : '（尚無資料）'}`)
lines.push(
  `- Moltbook：${
    latest.moltbook
...
lines.push('- CC股票研究工具 列表：`/reports/analysis/`')
lines.push('- Moltbook 列表：`/reports/moltbook/reports/`')
lines.push('')
lines.push('')
lines.push('排程（自動更新）：')
lines.push('')
lines.push('- 台股收盤：14:55 產生快取 → 15:00 Telegram 發送（<3500 字，分段）→ 18:00 更新三大法人到表格快取 → 18:10 寫入週檔（含摘要+表格）並 push')
lines.push('- 美股收盤：06:28 產生快取 → 06:35 Telegram 發送 → 06:40 寫入週檔並 push')
lines.push('- YT-澔哥：每日 11:10 摘要＋發送 Telegram＋寫入週檔並 push')
lines.push('- 財經新聞快報：每日 08:00 / 16:00 / 21:00（Asia/Taipei，近 5 小時｜台灣 1-5 則 AI 優先、6-8 則一般財經優先；國際 AI 優先；RSS 去重；每則只列標題、原文 link、可辨識相關股票）')
lines.push('')
